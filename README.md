...
- **任务下发尚未在真机上端到端跑通**：连接已验证，但「下发 → 执行 → 完成」的完整闭环还没跑过一次
- 小工具中的**识别类**功能（公招识别 / 干员识别 / 仓库识别）后端没有 API：界面按 MAA 桌面端还原、
  数据取自官方 JSON（公招库、干员表、物品表），但识别结果目前为本地模拟，页面已明确标注
- 自动战斗（Copilot）：后端无对应 API；界面为还原 + 本地模拟，作业文件可在本地解析
