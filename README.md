...
| 执行/停止（AsstStart/AsstStop） | ✅ 已实现 | 前端开始行动 + 停止按钮；没有任务链在跑时点停止/断开会立即收尾并释放会话（MaaCore 此时不会回 TaskChainStopped，靠回调收尾会永久卡住），任务运行中停止等当前任务收尾并带 15s 看门狗，再点一次为「强制停止」 |
...
| 基建换班 Infrast | ✅ 已实现 | 20 字段全接入：模式/设施/无人机/心情阈值/宿舍蹭信赖/菲亚梅塔/线索收取交流赠送/跨设施组合/训练室专精；界面语义值会在下发前归一化为协议值（manufacture→Mfg、manu_gold→PureGold、normal→0、阈值百分比→0~1），并做协议枚举兜底（此前 4 个字段取值非法，MaaCore 直接拒绝追加任务） |
...
| 全选 | ✅ 已实现 | 队列工具条：全选/清空/保存配置；队列（任务项 + 勾选状态 + 顺序）整体持久化到服务端 tasks.json 的 _meta.queue，刷新后不再回到硬编码默认勾选 |
| 等待 & 停止 | ✅ 已实现 | 开始/停止按钮 + 运行状态 2.5s 轮询 + 运行实况时间线（WS 日志）；停止后再点一次变「强制停止」，立即释放会话 |
