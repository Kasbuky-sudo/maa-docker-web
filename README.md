...
- Web 界面（windows-ui 官方组件体系，全部官方 dist 组件）：一键长草工作台（任务队列 / 配置面板 / 运行实况 / 状态栏）/ 日程 / 设置 / Runtime / 日志 / 功能对照 / 关于
- 连接测试（`AsstAsyncConnect` 探活，返回耗时）与「完成后动作」（MaaCore `CloseDown`：睡眠 / 休眠 / 关机）
...
- 连接设置已有地址 / ADB 路径 / 连接配置；缺触控模式、MuMu/LD 截图增强等细项
...
| 理智作战 Fight | 🟡 部分实现 | 协议 16 字段已接入（关卡/理智药与过期策略/碎石/次数/代理倍率/掉落上报/服务器/博朗台）；缺桌面端 GUI 级「周计划」与「材料目标库存」（非协议字段） |
...
| 全选 | ❌ 未实现 |  |
| 等待 & 停止 | ❌ 未实现 | 有立即停止 |
| 完成后动作（退出游戏/模拟器/关机/休眠…） | 🟡 部分实现 | 无操作/睡眠/休眠/关机走 MaaCore CloseDown（协议有文档）；退出游戏/模拟器为附加字段，未实机验证 |
| 任务超时提醒 | ❌ 未实现 |  |
| 今日关卡提示 | ❌ 未实现 |  |
| 自动重载资源 | ❌ 未实现 |  |
| 定时执行（日程） | 🟡 部分实现 | 日程 UI + 存储已实现；服务端调度器未接 |
