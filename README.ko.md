...
- **실기기에서 작업 실행의 종단 간 검증은 아직 없음**(연결은 확인, 다운로드→실행→완료 흐름은 미검증)
- 인식 도구(공개모집 / 오퍼레이터 / 창고)는 백엔드 API 가 없어 UI 는 데스크톱 재현 + 공식 JSON 데이터이며
  결과는 로컬 시뮬레이션(페이지에 명시)
- 자동 전투(Copilot): 백엔드 API 없음, UI 는 재현 + 로컬 시뮬레이션, 작업 파일은 로컬 파싱
- 연결 설정 외의 설정은 서버에 저장되지 않음(`config.json` 화이트리스트는 4개 필드)
- 일정은 저장만 되고 **서버 측 스케줄러는 미구현**
