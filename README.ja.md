...
- **実機でのタスク実行はまだ端到端で未検証**（接続は確認済み、実行の一連の流れは未実施）
- 認識系ツール（公開求人 / オペレーター / 倉庫）はバックエンド API がなく、UI はデスクトップ版の再現＋
  公式 JSON のデータ（求人データベース・オペレーター表・アイテム表）を使用。ただし結果はローカル模拟で、
  ページにその旨を明記
- 自動戦闘（Copilot）：バックエンド API なし。UI は再現＋ローカル模拟、ジョブファイルはローカル解析
- 接続設定以外の設定はサーバーに保存されません（`config.json` のホワイトリストは 4 項目）
- スケジュールは保存のみで、**サーバー側のスケジューラは未実装**
